...
# Theme version for the mirror watermark (`watermark.include_version`).
from .. import __version__ as _VOID_THEME_VERSION
from .. import social_card as _social_card

_AI_LOGGER = logging.getLogger("mkdocs.plugins.void")
...
#
# Maps `theme.void.<group>.<key>` to the CSS custom property it drives.
...
# specific pieces out from `theme.void.components.<name>.<key>`.
_VOID_DEFAULT_COMPONENTS = {
    ...
    "prefetch": {
        # Prefetch-on-hover (InstantLoading-style). ON by default.
...
# toggle_scheme, toggle_repo_popover, open_repo) is resolved in `void.js`.
...
# is configurable via `theme.void.reading_mode`; the defaults keep the
# feature fully ON.
...
# compiled-CSS token (--void-ink, --void-glass-bg, --void-text-primary,
# --void-text-secondary, --void-glass-border, --void-accent). Unknown
...
# behavioral aspect is configurable via `theme.void.action_cluster`; the
...
# Features built later (the timer) light up automatically.
...
_VOID_ACTION_CLUSTER_POSITIONS = ("bottom-left", "bottom-right")
_VOID_ACTION_CLUSTER_ICONS = (
    "plus",
    "menu",
    "notes",
    "help",
...
_VOID_ACTION_CLUSTER_ANIMATIONS = ("normal", "reduced", "none")
_VOID_ACTION_CLUSTER_IDS = (
    "keyboard_help",
    "notes",
    "timer",
    "reading_mode",
...
_VOID_ACTION_CLUSTER_OFFSET_KEYS = ("bottom", "left", "right")
_VOID_ACTION_CLUSTER_BADGES = ("none", "time")
_VOID_ACTION_CLUSTER_BEHAVIOR_BOOLS = (
    "close_on_select",
    "close_on_escape",
    "close_on_outside",
    "tooltips",
...
# Standalone mkdocs.yml config builder.
#
...
_VOID_CONFIG_BUILDER_BOOLS = ("enabled", "cluster_action", "toc_footer")
_VOID_CONFIG_BUILDER_OPEN_TARGETS = ("_blank", "_self")
...
# the visible surfaces. Every knob is `theme.void.timer` configurable and the
# defaults ship fully ON with a 25-minute Pomodoro-style default session.
# Adds the remaining-time badge in the cluster, the `mm:ss`/`m:ss`/`SS`
...
_VOID_TIMER_TOC_POSITIONS = ("top", "bottom")
_VOID_TIMER_TOC_STYLES = ("ring", "bar", "digits")
_VOID_TIMER_DISPLAY_FORMATS = ("mm:ss", "m:ss", "SS")

# Content area customization.
#
# `theme.void.content` gives full control over content rendering. Empty
...
# Social cards & structured data. `extra.void_og_image` set to the
...
_VOID_SOCIAL_CARDS_BOOLS = ("enabled", "jsonld", "cards")

# Page metadata bar: last-updated date + edit-on-GitHub link.
_VOID_DEFAULT_META = {
    "enabled": True,
...
_VOID_META_BOOLS = ("enabled", "show_last_updated", "show_edit_on_github")

# Engagement & privacy surfaces. Feedback opens a prefilled GitHub
...
_VOID_FEEDBACK_BOOLS = ("enabled", "show")
...
_VOID_ANNOUNCEMENT_BAR_BOOLS = ("enabled", "show", "dismissable")
...
_VOID_COOKIE_CONSENT_BOOLS = ("enabled", "show")
_VOID_COOKIE_CONSENT_RENDER = ("auto", "always", "never")
...
_VOID_FIXED_POSITIONS = ("top", "right", "bottom", "left", "center")

# Repo popover info sections. Every section ships by default; an
...
_VOID_COMMENTS_BOOLS = ("enabled",)
_VOID_COMMENTS_PROVIDERS = ("", "giscus")
_VOID_COMMENTS_MAPPINGS = ("pathname", "url", "title", "og:title", "specific")

# UI-string i18n. This is the single source of truth for the chrome
...
# `theme.void.i18n` — either the nested group form or the concise flat aliases
...
# Flat alias -> nested i18n group/child path. Keeps the documented
...
# Breadcrumbs. A trail above the content top whenever a page has
# ancestors; the toggle ships ON so nothing has to be configured.
_VOID_DEFAULT_BREADCRUMBS = {"show": True}
_VOID_BREADCRUMBS_BOOLS = ("show",)

# PWA identity. The plugin auto-generates `manifest.webmanifest` so a
# docs build is installable with the service worker the theme already ships.
# `theme_color` / `start_url` fall back to `extra.void_theme_color` /
...
_VOID_PWA_BOOLS = ("manifest", "icons")
_VOID_PWA_DISPLAYS = ("standalone", "fullscreen", "minimal-ui", "browser")

# Asset referencing (`cdn | local | bundle`) + inline critical CSS.
#
...
_VOID_ASSET_MODES = frozenset({"cdn", "local", "bundle"})
...
_VOID_ASSET_BOOLS = ("inline_critical_css",)
...
# what a `cdn` build would have loaded (the same strings `void.js` and
...
_VOID_AI_READER_URL_STYLES = ("sidecar", "inline")
_VOID_AI_READER_BOOLS = (
    "enabled",
    "markup",
    "overwrite",
    "auto_title",
...
_VOID_AI_READER_WATERMARK_BOOLS = (
    "enabled",
    "header",
    "footer",
    "include_site",
...
def _normalize_i18n_overrides(provided):
    """Fold the concise flat alias keys (`theme.void.i18n.search_placeholder`)
    ...
    for key, value in provided.items():
        alias = _VOID_I18N_FLAT_ALIASES.get(key)
...
def _normalize_admonition_icons(content):
    ...
    `void-content-tokens` <style>. A raw `url(...)` fails CSS tokenization
...
def _validate_shortcut_entry(name, entry):
    """Validate a single `theme.void.keyboard.shortcuts.<name>` mapping."""
    if not isinstance(entry, dict):
        raise ConfigurationError(
            f"theme.void.keyboard.shortcuts.{name} must be a mapping with "
            "a 'key' and 'label'."
        )
    ...
    if key_value is not None and (
        ...
        raise ConfigurationError(
            f"theme.void.keyboard.shortcuts.{name}.key must be a non-empty string."
        )
    label_value = entry.get("label")
    if label_value is not None and not isinstance(label_value, str):
        raise ConfigurationError(
            f"theme.void.keyboard.shortcuts.{name}.label must be a string."
        )
    for field in ("enabled", "persisted"):
        value = entry.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.keyboard.shortcuts.{name}.{field} must be a boolean."
            )


def _validate_keyboard(keyboard):
    """Validate a merged `theme.void.keyboard` mapping, raising a clear
    ...
    if not isinstance(custom, list):
        raise ConfigurationError("theme.void.keyboard.custom must be a list.")
    for index, entry in enumerate(custom):
        if not isinstance(entry, dict):
            raise ConfigurationError(
                f"theme.void.keyboard.custom[{index}] must be a mapping with "
        ...
        for field in ("key", "action", "label"):
            ...
            if value is not None and not isinstance(value, str):
                raise ConfigurationError(
                    f"theme.void.keyboard.custom[{index}].{field} must be a string."
                )
        if not isinstance(entry.get("action"), str) or not entry["action"].strip():
            raise ConfigurationError(
                f"theme.void.keyboard.custom[{index}].action is required."
            )


def _validate_reading_mode(reading_mode):
    """Validate a merged `theme.void.reading_mode` mapping, raising a clear
    MkDocs configuration error for malformed entries instead of silently
    degrading the reading view."""
    if not isinstance(reading_mode, dict):
        raise ConfigurationError("theme.void.reading_mode must be a mapping.")

    for field in ("enabled", "persisted"):
        value = reading_mode.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.reading_mode.{field} must be a boolean."
            )

    shortcut = reading_mode.get("shortcut_key")
    if shortcut is not None and (not isinstance(shortcut, str) or not shortcut.strip()):
        raise ConfigurationError(
            "theme.void.reading_mode.shortcut_key must be a non-empty string."
        )
    ...
    if isinstance(sections, dict):
        for name in ("header", "sidebar", "toc", "footer", "progress"):
            ...
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.reading_mode.sections.{name} must be a boolean."
                )
    elif sections is not None:
        raise ConfigurationError("theme.void.reading_mode.sections must be a mapping.")
    ...
    if isinstance(notes, dict):
        for field in ("show", "open_on_enter"):
            ...
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.reading_mode.notes.{field} must be a boolean."
                )
    elif notes is not None:
        raise ConfigurationError("theme.void.reading_mode.notes must be a mapping.")
    ...
    if isinstance(scheme, dict):
        ...
        if enabled is not None and not isinstance(enabled, bool):
            raise ConfigurationError(
                "theme.void.reading_mode.scheme.enabled must be a boolean."
            )
        name = scheme.get("name")
        if name is not None and (not isinstance(name, str) or not name.strip()):
            raise ConfigurationError(
                "theme.void.reading_mode.scheme.name must be a non-empty string."
            )
        colors = scheme.get("colors")
        if colors is not None:
            if not isinstance(colors, dict):
                raise ConfigurationError(
                    "theme.void.reading_mode.scheme.colors must be a mapping."
                )
            for key in colors:
                if key not in _VALID_READING_COLOR_KEYS:
                    raise ConfigurationError(
                        f"theme.void.reading_mode.scheme.colors.{key} is not a "
                        "valid color key; expected one of "
                        f"{sorted(_VALID_READING_COLOR_KEYS)}."
                    )
    elif scheme is not None:
        raise ConfigurationError("theme.void.reading_mode.scheme must be a mapping.")
    ...
    if isinstance(typography, dict):
        for field in ("font_size", "line_height", "measure"):
            ...
            if value is not None and (not isinstance(value, str) or not value.strip()):
                raise ConfigurationError(
                    f"theme.void.reading_mode.typography.{field} must be a "
                    "non-empty string."
                )
    elif typography is not None:
        raise ConfigurationError(
            "theme.void.reading_mode.typography must be a mapping."
        )


def _validate_action_cluster(action_cluster):
    """Validate a merged `theme.void.action_cluster` mapping, raising a clear
    MkDocs configuration error for malformed entries instead of silently
    degrading the plus menu."""
    if not isinstance(action_cluster, dict):
        raise ConfigurationError("theme.void.action_cluster must be a mapping.")

    enabled = action_cluster.get("enabled")
    if enabled is not None and not isinstance(enabled, bool):
        raise ConfigurationError("theme.void.action_cluster.enabled must be a boolean.")

    position = action_cluster.get("position")
    if position is not None and position not in _VOID_ACTION_CLUSTER_POSITIONS:
        raise ConfigurationError(
            f"theme.void.action_cluster.position must be one of "
            f"{sorted(_VOID_ACTION_CLUSTER_POSITIONS)}; got {position!r}."
        )

    offset = action_cluster.get("offset")
    if isinstance(offset, dict):
        for key, value in offset.items():
            if key not in _VOID_ACTION_CLUSTER_OFFSET_KEYS:
                raise ConfigurationError(
                    f"theme.void.action_cluster.offset.{key} is not a valid "
                    "offset key; expected one of "
                    f"{sorted(_VOID_ACTION_CLUSTER_OFFSET_KEYS)}."
                )
            if not isinstance(value, str) or not value.strip():
                raise ConfigurationError(
                    f"theme.void.action_cluster.offset.{key} must be a "
                    "non-empty string."
                )
    elif offset is not None:
        raise ConfigurationError("theme.void.action_cluster.offset must be a mapping.")

    main = action_cluster.get("main")
    if isinstance(main, dict):
        icon = main.get("icon")
        if icon is not None and icon not in _VOID_ACTION_CLUSTER_ICONS:
            raise ConfigurationError(
                f"theme.void.action_cluster.main.icon must be one of "
                f"{sorted(_VOID_ACTION_CLUSTER_ICONS)}; got {icon!r}."
            )
        size = main.get("size")
        if size is not None and (not isinstance(size, str) or not size.strip()):
            raise ConfigurationError(
                "theme.void.action_cluster.main.size must be a non-empty string."
            )
        for field in ("glass", "icon_transform"):
            value = main.get(field)
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.action_cluster.main.{field} must be a boolean."
                )
    elif main is not None:
        raise ConfigurationError("theme.void.action_cluster.main must be a mapping.")
    ...
    if isinstance(behavior, dict):
        ...
        if min_actions is not None and (
            ...
            raise ConfigurationError(
                "theme.void.action_cluster.behavior.min_actions must be an integer."
            )
        animation = behavior.get("animation")
        if animation is not None and animation not in _VOID_ACTION_CLUSTER_ANIMATIONS:
            raise ConfigurationError(
                f"theme.void.action_cluster.behavior.animation must be one of "
                f"{sorted(_VOID_ACTION_CLUSTER_ANIMATIONS)}; got {animation!r}."
            )
        for field in _VOID_ACTION_CLUSTER_BEHAVIOR_BOOLS:
            value = behavior.get(field)
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.action_cluster.behavior.{field} must be a boolean."
                )
    elif behavior is not None:
        raise ConfigurationError(
            "theme.void.action_cluster.behavior must be a mapping."
        )

    replaces = action_cluster.get("replaces_notes_button")
    if replaces is not None and not isinstance(replaces, bool):
        raise ConfigurationError(
            "theme.void.action_cluster.replaces_notes_button must be a boolean."
        )
    ...
    if actions is not None:
        if not isinstance(actions, list):
            raise ConfigurationError(
                "theme.void.action_cluster.actions must be a list."
            )
        seen = set()
        for index, entry in enumerate(actions):
            if not isinstance(entry, dict):
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}] must be a "
                    "mapping with 'id', 'icon', 'label', 'shortcut', 'badge', "
                    "and 'enabled'."
                )
            action_id = entry.get("id")
            if action_id not in _VOID_ACTION_CLUSTER_IDS:
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].id is not a "
                    "known action id; expected one of "
                    f"{sorted(_VOID_ACTION_CLUSTER_IDS)}."
                )
            if action_id in seen:
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].id duplicates "
            ...
            if icon is not None and icon not in _VOID_ACTION_CLUSTER_ICONS:
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].icon must be "
                    f"one of {sorted(_VOID_ACTION_CLUSTER_ICONS)}; got {icon!r}."
                )
            label_value = entry.get("label")
            if label_value is not None and (
                ...
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].label must be "
                    "a non-empty string."
                )
            ...
            if shortcut is not None and (
                ...
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].shortcut must "
                    "be a non-empty key string (e.g. 'Alt+Shift+T')."
                )
            badge = entry.get("badge")
            if badge is not None and badge not in _VOID_ACTION_CLUSTER_BADGES:
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].badge must be "
                    f"one of {sorted(_VOID_ACTION_CLUSTER_BADGES)}; got "
                    f"{badge!r}."
                )
            action_enabled = entry.get("enabled")
            if action_enabled is not None and not isinstance(action_enabled, bool):
                raise ConfigurationError(
                    f"theme.void.action_cluster.actions[{index}].enabled must "
                    "be a boolean."
                )


def _validate_config_builder(config_builder):
    """Validate a merged `theme.void.config_builder` mapping. The tool is a
    ...
    if not isinstance(config_builder, dict):
        raise ConfigurationError("theme.void.config_builder must be a mapping.")

    for field in _VOID_CONFIG_BUILDER_BOOLS:
        value = config_builder.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.config_builder.{field} must be a boolean."
            )
    ...
    if url is not None and (
        ...
        raise ConfigurationError(
            "theme.void.config_builder.url must be a non-empty, slash-less "
            "path relative to the site root (e.g. 'assets/config-builder.html')."
        )

    open_target = config_builder.get("open_target")
    if open_target is not None and open_target not in _VOID_CONFIG_BUILDER_OPEN_TARGETS:
        raise ConfigurationError(
            f"theme.void.config_builder.open_target must be one of "
            f"{sorted(_VOID_CONFIG_BUILDER_OPEN_TARGETS)}; got {open_target!r}."
        )


def _validate_timer(timer):
    """Validate a merged `theme.void.timer` mapping, raising a clear MkDocs
    configuration error for malformed entries instead of silently degrading
    the focus timer."""
    if not isinstance(timer, dict):
        raise ConfigurationError("theme.void.timer must be a mapping.")

    enabled = timer.get("enabled")
    if enabled is not None and not isinstance(enabled, bool):
        raise ConfigurationError("theme.void.timer.enabled must be a boolean.")
    ...
    if default_minutes is not None and (
        ...
        raise ConfigurationError(
            "theme.void.timer.default_minutes must be a positive integer."
        )
    ...
    if isinstance(toc, dict):
        ...
        if toc_show is not None and not isinstance(toc_show, bool):
            raise ConfigurationError("theme.void.timer.toc.show must be a boolean.")
        position = toc.get("position")
        if position is not None and position not in _VOID_TIMER_TOC_POSITIONS:
            raise ConfigurationError(
                f"theme.void.timer.toc.position must be one of "
                f"{sorted(_VOID_TIMER_TOC_POSITIONS)}; got {position!r}."
            )
        style = toc.get("style")
        if style is not None and style not in _VOID_TIMER_TOC_STYLES:
            raise ConfigurationError(
                f"theme.void.timer.toc.style must be one of "
                f"{sorted(_VOID_TIMER_TOC_STYLES)}; got {style!r}."
            )
    elif toc is not None:
        raise ConfigurationError("theme.void.timer.toc must be a mapping.")
    ...
    if isinstance(reading, dict):
        ...
        if reading_show is not None and not isinstance(reading_show, bool):
            raise ConfigurationError("theme.void.timer.reading.show must be a boolean.")
    elif reading is not None:
        raise ConfigurationError("theme.void.timer.reading must be a mapping.")
    ...
    if isinstance(notifications, dict):
        for field in ("enabled", "toast", "sound"):
            ...
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.timer.notifications.{field} must be a boolean."
                )
    elif notifications is not None:
        raise ConfigurationError("theme.void.timer.notifications must be a mapping.")
    ...
    for field in (
        ...
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.timer.{field} must be a boolean.")

    display_format = timer.get("display_format")
    if display_format is not None and display_format not in _VOID_TIMER_DISPLAY_FORMATS:
        raise ConfigurationError(
            f"theme.void.timer.display_format must be one of "
            f"{sorted(_VOID_TIMER_DISPLAY_FORMATS)}; got {display_format!r}."
        )
    ...
    if isinstance(colors, dict):
        ...
        if progress is not None and (
            ...
            raise ConfigurationError(
                "theme.void.timer.colors.progress must be a non-empty string."
            )
    elif colors is not None:
        raise ConfigurationError("theme.void.timer.colors must be a mapping.")


def _validate_ai_reader(ai_reader):
    """Validate a merged `theme.void.ai_reader` mapping, raising a clear MkDocs
    ...
    if not isinstance(ai_reader, dict):
        raise ConfigurationError("theme.void.ai_reader must be a mapping.")

    for field in _VOID_AI_READER_BOOLS:
        value = ai_reader.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.ai_reader.{field} must be a boolean.")

    url_style = ai_reader.get("url_style")
    if url_style is not None and url_style not in _VOID_AI_READER_URL_STYLES:
        raise ConfigurationError(
            f"theme.void.ai_reader.url_style must be one of "
            f"{sorted(_VOID_AI_READER_URL_STYLES)}; got {url_style!r}."
        )

    description = ai_reader.get("description")
    if description is not None and not isinstance(description, str):
        raise ConfigurationError("theme.void.ai_reader.description must be a string.")

    exclude = ai_reader.get("exclude")
    if exclude is not None:
        if not isinstance(exclude, list):
            raise ConfigurationError(
                "theme.void.ai_reader.exclude must be a list of glob strings "
                '(e.g. "drafts/*.md").'
            )
        for pattern in exclude:
            if not isinstance(pattern, str) or not pattern.strip():
                raise ConfigurationError(
                    "theme.void.ai_reader.exclude entries must be non-empty "
                    'glob strings (e.g. "drafts/*.md").'
                )

    watermark = ai_reader.get("watermark")
    if isinstance(watermark, dict):
        for field in _VOID_AI_READER_WATERMARK_BOOLS:
            value = watermark.get(field)
            if value is not None and not isinstance(value, bool):
                raise ConfigurationError(
                    f"theme.void.ai_reader.watermark.{field} must be a boolean."
                )
        if watermark.get("enabled"):
            text = watermark.get("text")
            if not isinstance(text, str) or not text.strip():
                raise ConfigurationError(
                    "theme.void.ai_reader.watermark.text must be a non-empty "
                    "string when watermark.enabled is true."
                )
    elif watermark is not None:
        raise ConfigurationError("theme.void.ai_reader.watermark must be a mapping.")


def _validate_social_cards(social_cards):
    """Validate a merged `theme.void.social_cards` mapping, raising a clear MkDocs
    configuration error for malformed entries instead of silently degrading the
    social-card / structured-data surface."""
    if not isinstance(social_cards, dict):
        raise ConfigurationError("theme.void.social_cards must be a mapping.")

    for field in _VOID_SOCIAL_CARDS_BOOLS:
        value = social_cards.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.social_cards.{field} must be a boolean."
            )


def _validate_meta(meta):
    """Validate a merged `theme.void.meta` mapping, raising a clear MkDocs
    ...
    if not isinstance(meta, dict):
        raise ConfigurationError("theme.void.meta must be a mapping.")

    for field in _VOID_META_BOOLS:
        value = meta.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.meta.{field} must be a boolean.")

    date_source = meta.get("date_source")
    if date_source not in (None, "", "auto", "front_matter", "git"):
        raise ConfigurationError(
            "theme.void.meta.date_source must be one of: 'auto', 'front_matter', 'git'."
        )

    for field in ("branch", "source_dir"):
        value = meta.get(field)
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(f"theme.void.meta.{field} must be a string.")
...
def _validate_feedback(feedback):
    """Validate a merged `theme.void.feedback` mapping, raising a clear MkDocs
    configuration error for malformed entries instead of silently dropping the
    shared-issue feedback widget."""
    if not isinstance(feedback, dict):
        raise ConfigurationError("theme.void.feedback must be a mapping.")

    for field in _VOID_FEEDBACK_BOOLS:
        value = feedback.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.feedback.{field} must be a boolean.")

    for field in ("title", "positive", "negative"):
        value = feedback.get(field)
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(f"theme.void.feedback.{field} must be a string.")

    labels = feedback.get("github_labels")
    if labels is not None:
        if not isinstance(labels, list):
            raise ConfigurationError(
                "theme.void.feedback.github_labels must be a list of "
                "GitHub label strings."
            )
        for label in labels:
            if not isinstance(label, str) or not label.strip():
                raise ConfigurationError(
                    "theme.void.feedback.github_labels entries must be "
                    "non-empty strings."
                )


def _validate_announcement_bar(announcement_bar):
    """Validate a merged `theme.void.announcement_bar` mapping, raising a clear
    MkDocs configuration error for malformed entries instead of silently dropping
    the announcement bar."""
    if not isinstance(announcement_bar, dict):
        raise ConfigurationError("theme.void.announcement_bar must be a mapping.")

    for field in _VOID_ANNOUNCEMENT_BAR_BOOLS:
        value = announcement_bar.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.announcement_bar.{field} must be a boolean."
            )

    text = announcement_bar.get("text")
    if text is not None and not isinstance(text, str):
        raise ConfigurationError("theme.void.announcement_bar.text must be a string.")

    position = announcement_bar.get("position")
    if position is not None and position not in _VOID_FIXED_POSITIONS:
        raise ConfigurationError(
            "theme.void.announcement_bar.position must be one of "
            f"{sorted(_VOID_FIXED_POSITIONS)}; got {position!r}."
        )


def _validate_cookie_consent(cookie_consent):
    """Validate a merged `theme.void.cookie_consent` mapping, raising a clear
    MkDocs configuration error for malformed entries instead of silently dropping
    the consent banner."""
    if not isinstance(cookie_consent, dict):
        raise ConfigurationError("theme.void.cookie_consent must be a mapping.")

    for field in _VOID_COOKIE_CONSENT_BOOLS:
        value = cookie_consent.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.cookie_consent.{field} must be a boolean."
            )
    ...
    for field in ("message", "accept_label", "decline_label", "privacy_policy"):
        ...
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(
                f"theme.void.cookie_consent.{field} must be a string."
            )

    render = cookie_consent.get("render")
    if render not in (None,) + _VOID_COOKIE_CONSENT_RENDER:
        raise ConfigurationError(
            "theme.void.cookie_consent.render must be one of "
            f"{sorted(_VOID_COOKIE_CONSENT_RENDER)}; got {render!r}."
        )

    position = cookie_consent.get("position")
    if position is not None and position not in _VOID_FIXED_POSITIONS:
        raise ConfigurationError(
            "theme.void.cookie_consent.position must be one of "
            f"{sorted(_VOID_FIXED_POSITIONS)}; got {position!r}."
        )


def _validate_repo_popover(repo_popover):
    """Validate a merged `theme.void.components.repo_popover` mapping. `show`
    ...
    if not isinstance(repo_popover, dict):
        raise ConfigurationError(
            "theme.void.components.repo_popover must be a mapping."
        )

    show = repo_popover.get("show")
    if show is not None and not isinstance(show, bool):
        raise ConfigurationError(
            "theme.void.components.repo_popover.show must be a boolean."
        )
    ...
    if fields is not None:
        if not isinstance(fields, (list, tuple)):
            raise ConfigurationError(
                "theme.void.components.repo_popover.fields must be a list of "
                "field names."
            )
        for field in fields:
            if not isinstance(field, str) or field not in _REPO_POPOVER_FIELDS:
                raise ConfigurationError(
                    "theme.void.components.repo_popover.fields contains an "
...
def _validate_comments(comments):
    """Validate a merged `theme.void.comments` mapping, raising a clear MkDocs
    configuration error for malformed entries instead of silently dropping the
    giscus comments integration."""
    if not isinstance(comments, dict):
        raise ConfigurationError("theme.void.comments must be a mapping.")

    for field in _VOID_COMMENTS_BOOLS:
        value = comments.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.comments.{field} must be a boolean.")

    provider = comments.get("provider")
    if provider not in (None, "") and provider not in _VOID_COMMENTS_PROVIDERS:
        raise ConfigurationError(
            "theme.void.comments.provider must be one of "
            f"{sorted(_VOID_COMMENTS_PROVIDERS)}; got {provider!r}."
        )

    mapping = comments.get("mapping")
    if mapping not in (None, "") and mapping not in _VOID_COMMENTS_MAPPINGS:
        raise ConfigurationError(
            "theme.void.comments.mapping must be one of "
            f"{sorted(_VOID_COMMENTS_MAPPINGS)}; got {mapping!r}."
        )

    for field in ("repo", "repo_id", "category", "category_id", "term", "language"):
        value = comments.get(field)
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(f"theme.void.comments.{field} must be a string.")

    repo = comments.get("repo") or ""
    if isinstance(repo, str) and repo.strip() and "/" not in repo:
        raise ConfigurationError(
            "theme.void.comments.repo must be in the 'owner/repo' form "
    ...
    if isinstance(theme, dict):
        for field in ("light", "dark"):
            ...
            if value is not None and not isinstance(value, str):
                raise ConfigurationError(
                    f"theme.void.comments.theme.{field} must be a string."
                )
    elif theme is not None:
        raise ConfigurationError("theme.void.comments.theme must be a mapping.")


_VOID_GLASS_VALUES = ("light", "medium", "heavy", "none")
_VOID_ANIMATION_VALUES = ("normal", "reduced", "none")
_VOID_BORDER_VALUES = ("none", "thin", "thick")


def _validate_i18n(i18n):
    """Validate a merged `theme.void.i18n` mapping. Values are nested string
    ...
    if not isinstance(i18n, dict):
        raise ConfigurationError("theme.void.i18n must be a mapping.")
        ...
        if isinstance(value, dict):
            for child_key, child_value in value.items():
                if not isinstance(child_value, str):
                    raise ConfigurationError(
                        f"theme.void.i18n.{key}.{child_key} must be a string."
                    )
            continue
        if isinstance(value, str):
            raise ConfigurationError(
                "theme.void.i18n contains an unknown flat key "
                f"{key!r}; supported flat keys: "
                f"{', '.join(sorted(_VOID_I18N_FLAT_ALIASES))}."
            )
        raise ConfigurationError(
            f"theme.void.i18n.{key} must be a string or a mapping."
        )


def _validate_breadcrumbs(breadcrumbs):
    """Validate a merged `theme.void.breadcrumbs` mapping, raising a clear
    MkDocs configuration error for a malformed `show` toggle."""
    if not isinstance(breadcrumbs, dict):
        raise ConfigurationError("theme.void.breadcrumbs must be a mapping.")

    for field in _VOID_BREADCRUMBS_BOOLS:
        value = breadcrumbs.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(
                f"theme.void.breadcrumbs.{field} must be a boolean."
            )


def _validate_pwa(pwa):
    """Validate a merged `theme.void.pwa` mapping. `manifest`/`icons` gates
    ...
    if not isinstance(pwa, dict):
        raise ConfigurationError("theme.void.pwa must be a mapping.")

    for field in _VOID_PWA_BOOLS:
        value = pwa.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.pwa.{field} must be a boolean.")

    display = pwa.get("display")
    if display not in (None,) + _VOID_PWA_DISPLAYS:
        raise ConfigurationError(
            "theme.void.pwa.display must be one of "
            f"{sorted(_VOID_PWA_DISPLAYS)}; got {display!r}."
        )

    for field in ("theme_color", "background_color", "start_url"):
        value = pwa.get(field)
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(f"theme.void.pwa.{field} must be a string.")


def _validate_assets(assets):
    """Validate a merged `theme.void.assets` mapping. `mode` gates how the
    ...
    if not isinstance(assets, dict):
        raise ConfigurationError("theme.void.assets must be a mapping.")

    for field in _VOID_ASSET_BOOLS:
        value = assets.get(field)
        if value is not None and not isinstance(value, bool):
            raise ConfigurationError(f"theme.void.assets.{field} must be a boolean.")

    mode = assets.get("mode")
    if mode not in (None,) + tuple(sorted(_VOID_ASSET_MODES)):
        raise ConfigurationError(
            "theme.void.assets.mode must be one of "
            f"{sorted(_VOID_ASSET_MODES)}; got {mode!r}."
        )

    for field in ("vendor_dir",):
        value = assets.get(field)
        if value is not None and not isinstance(value, str):
            raise ConfigurationError(f"theme.void.assets.{field} must be a string.")

    timeout = assets.get("timeout")
    if timeout is not None and (not isinstance(timeout, int) or timeout <= 0):
        raise ConfigurationError(
            "theme.void.assets.timeout must be a positive integer."
        )
...
def _validate_visual_options(void, plugin_options):
    """Validate and normalize the merged top-level visual options.

    Runs on the single merged surface (``theme.void`` + ``plugins.void``)
    ...
    def label(key):
        base = "plugins.void" if key in plugin_options else "theme.void"
        return f"{base}.{key}"

    if "dot_matrix" in void:
        dot = void["dot_matrix"]
        if isinstance(dot, dict):
            ...
            void["dot_matrix"] = _coerce_bool(dot, label("dot_matrix"))

    for key in ("highlight", "notes"):
        if key in void:
            void[key] = _coerce_bool(void[key], label(key))

    notes_ttl = void.get("notes_ttl")
    ...
    visual = {
        "glass": ("intensity", _VOID_GLASS_VALUES),
        "animation": ("mode", _VOID_ANIMATION_VALUES),
        "border": ("width", _VOID_BORDER_VALUES),
    }
...
class VoidPlugin(BasePlugin):
    """Plugin that enhances the Void theme with additional context."""

    # Every `theme.void` key is also accepted as a `- void:`
    # plugin option, plus the plugin-specific scalars (highlight, notes,
    # notes_ttl). Options given here win over `theme.void`. Option entries
    # deliberately carry no default: an absent key stays `None` and is skipped
    # during the merge so defaults keep flowing from `theme.void`.
    ...
    _void_defaults: ClassVar[dict[str, object]] = {
        "glass": "medium",
        "dot_matrix": True,
        "animation": "normal",
        "border": "thin",
    ...
    def on_config(self, config, **kwargs):
        ...
        void = theme.get("void") if "void" in theme else {}
        ...
        # The `- void:` plugin options are a first-class settings
        # surface. Keys supplied inline in the plugin dict are deep-merged over
        # `theme.void` (the plugin wins), then the merged result is validated
        ...
        void = _deep_merge(void, plugin_opts)
        _validate_visual_options(void, plugin_opts)
        for key, value in self._void_defaults.items():
            void.setdefault(key, value)
        theme["void"] = void

        # Resolve component visibility toggles. User overrides are
        ...
        # Resolve reading-mode settings. Defaults are all-ON
        # (distraction-free view + Ink palette); user overrides are deep-merged
        # and validated, then the shortcut is seeded into the keyboard layer.
        provided_reading = void.get("reading_mode")
        if not isinstance(provided_reading, dict):
            provided_reading = {}
        reading_mode = _deep_merge(_VOID_DEFAULT_READING_MODE, provided_reading)
        _validate_reading_mode(reading_mode)
        void["reading_mode"] = reading_mode
        theme["void"] = void

        # Resolve the action cluster (plus menu). Defaults ship all
        # four action slots ON (`keyboard_help`, `notes`, `timer`,
        # `reading_mode`); user overrides are deep-merged and validated, then
        # the toggle shortcut is seeded into the keyboard layer below so an
        # explicit `keyboard.shortcuts.toggle_action_cluster` the author set
        # still wins.
        provided_action_cluster = void.get("action_cluster")
        if not isinstance(provided_action_cluster, dict):
            provided_action_cluster = {}
        action_cluster = _deep_merge(
            _VOID_DEFAULT_ACTION_CLUSTER, provided_action_cluster
        )
        _validate_action_cluster(action_cluster)

        # Wire the standalone config builder. The tool itself is a
        ...
        provided_config_builder = void.get("config_builder")
        if not isinstance(provided_config_builder, dict):
            provided_config_builder = {}
        config_builder = _deep_merge(
            _VOID_DEFAULT_CONFIG_BUILDER, provided_config_builder
        )
        ...
        void["action_cluster"] = action_cluster
        void["config_builder"] = config_builder
        theme["void"] = void

        # Resolve the focus timer. Defaults ship fully ON (TOC
        ...
        # into the keyboard layer below so an explicit
        # `keyboard.shortcuts.timer_toggle` the author set still wins.
        provided_timer = void.get("timer")
        if not isinstance(provided_timer, dict):
            provided_timer = {}
        timer = _deep_merge(_VOID_DEFAULT_TIMER, provided_timer)
        _validate_timer(timer)
        void["timer"] = timer
        theme["void"] = void

        # Resolve the AI-readable content mode. The whole surface
        ...
        provided_ai_reader = void.get("ai_reader")
        if not isinstance(provided_ai_reader, dict):
            provided_ai_reader = {}
        ai_reader = _deep_merge(_VOID_DEFAULT_AI_READER, provided_ai_reader)
        _validate_ai_reader(ai_reader)
        void["ai_reader"] = ai_reader
        theme["void"] = void

        # Resolve social cards & structured data. The whole surface
        # (per-page Article JSON-LD + auto card images) ships ON by default;
        # `cards` only fires when `extra.void_og_image` is also `__auto__`.
        provided_social_cards = void.get("social_cards")
        if not isinstance(provided_social_cards, dict):
            provided_social_cards = {}
        social_cards = _deep_merge(_VOID_DEFAULT_SOCIAL_CARDS, provided_social_cards)
        _validate_social_cards(social_cards)
        void["social_cards"] = social_cards
        theme["void"] = void

        # Page metadata bar: last-updated date + edit-on-GitHub link.
        provided_meta = void.get("meta")
        if not isinstance(provided_meta, dict):
            provided_meta = {}
        meta = _deep_merge(_VOID_DEFAULT_META, provided_meta)
        _validate_meta(meta)
        void["meta"] = meta
        theme["void"] = void

        # Engagement & privacy: shared-issue feedback widget. The PRD
        # config uses `show: true`; the master `enabled` switch keeps the surface
        # consistent with every other Void feature that ships ON by default.
        provided_feedback = void.get("feedback")
        if not isinstance(provided_feedback, dict):
            provided_feedback = {}
        feedback = _deep_merge(_VOID_DEFAULT_FEEDBACK, provided_feedback)
        _validate_feedback(feedback)
        void["feedback"] = feedback
        theme["void"] = void

        # Announcement bar. `announcement_bar.text` is the primary
        # source; the legacy `extra.void_announce` string from the PRD config
        # block is honored as a convenience alias when the dict text is unset.
        provided_announcement_bar = void.get("announcement_bar")
        if not isinstance(provided_announcement_bar, dict):
            provided_announcement_bar = {}
        announcement_bar = _deep_merge(
            _VOID_DEFAULT_ANNOUNCEMENT_BAR, provided_announcement_bar
        )
        _validate_announcement_bar(announcement_bar)
        if not announcement_bar["text"]:
            legacy_announce = (config.get("extra") or {}).get("void_announce")
            if isinstance(legacy_announce, str) and legacy_announce.strip():
                announcement_bar["text"] = legacy_announce.strip()
        void["announcement_bar"] = announcement_bar
        theme["void"] = void

        # Cookie consent. The banner is purely informational and
        ...
        provided_cookie_consent = void.get("cookie_consent")
        if not isinstance(provided_cookie_consent, dict):
            provided_cookie_consent = {}
        cookie_consent = _deep_merge(
            _VOID_DEFAULT_COOKIE_CONSENT, provided_cookie_consent
        )
        _validate_cookie_consent(cookie_consent)
        void["cookie_consent"] = cookie_consent
        theme["void"] = void

        # Comments. giscus is injected only when `repo` + `repo_id`
        # are configured; everything else (labels, mapping, theme sync) is
        # additive and optional.
        provided_comments = void.get("comments")
        if not isinstance(provided_comments, dict):
            provided_comments = {}
        comments = _deep_merge(_VOID_DEFAULT_COMMENTS, provided_comments)
        _validate_comments(comments)
        void["comments"] = comments
        theme["void"] = void

        # UI-string i18n. Defaults carry the complete English chrome
        ...
        provided_i18n = void.get("i18n")
        if not isinstance(provided_i18n, dict):
            provided_i18n = {}
        i18n = _deep_merge(_VOID_DEFAULT_I18N, _normalize_i18n_overrides(provided_i18n))
        _validate_i18n(i18n)
        void["i18n"] = i18n
        theme["void"] = void

        # Breadcrumbs. Renders above the content top whenever a page
        # has ancestors; the toggle ships ON by default.
        provided_breadcrumbs = void.get("breadcrumbs")
        if not isinstance(provided_breadcrumbs, dict):
            provided_breadcrumbs = {}
        breadcrumbs_cfg = _deep_merge(_VOID_DEFAULT_BREADCRUMBS, provided_breadcrumbs)
        _validate_breadcrumbs(breadcrumbs_cfg)
        void["breadcrumbs"] = breadcrumbs_cfg
        theme["void"] = void

        # PWA identity. Auto-generated manifest + app meta ships ON by
        # default; theme_color/start_url/icons fall back to site config when the
        # author leaves them empty.
        provided_pwa = void.get("pwa")
        if not isinstance(provided_pwa, dict):
            provided_pwa = {}
        pwa = _deep_merge(_VOID_DEFAULT_PWA, provided_pwa)
        _validate_pwa(pwa)
        void["pwa"] = pwa
        theme["void"] = void

        # Asset referencing (`cdn | local | bundle`) + inline critical
        ...
        provided_assets = void.get("assets")
        if not isinstance(provided_assets, dict):
            provided_assets = {}
        assets = _deep_merge(_VOID_DEFAULT_ASSETS, provided_assets)
        ...
        if mode in ("local", "bundle"):
            ...
            if not vendor:
                vendor = str(_VOID_DEFAULT_ASSETS["vendor_dir"]).strip("/")
            vendor = vendor + "/"
            ...
            bundle_js = f"{vendor}bundle/void-offline.js"
            hljs_cfg["cdn_url"] = (
            ...
            _AI_LOGGER.warning(
                "void assets: inline_critical_css only applies when "
                "assets.mode is 'local' or 'bundle'; ignored under 'cdn'."
            )

        # Consent gating. Void ships no trackers, so the consent
        ...
        # Mirror bookkeeping for the build hooks (a fresh build always
        # resets both so a plugin instance is never reused across builds).
        self._ai_mirrors = []
        self._ai_claimed = set()
        self._ai_warned_overwrite = False
        # Bookkeeping: per-page social cards resolved during the render;
        ...
        # Mirror `timer.start_with_reading` into the reading-mode
        ...
        # Resolve keyboard shortcuts. Defaults are all-ON (every
        ...
        provided_keyboard = void.get("keyboard")
        if not isinstance(provided_keyboard, dict):
            provided_keyboard = {}

        # Seed the reading-mode shortcut from `reading_mode` before
        # The deep-merge, so an explicit
        ...
        seeded_keyboard["shortcuts"].setdefault(
            "toggle_reading_mode",
            {
                ...
                or _VOID_DEFAULT_READING_MODE["shortcut_key"],
                "label": "Toggle reading mode",
        ...
        # Seed the action-cluster shortcut from `action_cluster` so
        ...
        # Seed the focus-timer shortcut from `timer` so an explicit
        ...
        keyboard = _deep_merge(_VOID_DEFAULT_KEYBOARD, seeded_keyboard)
        ...
        # C1-C3: surface the void theme options to templates so they can be
        # applied to the rendered output (glass, dot matrix, animation, border).
        extra = config.get("extra") or {}
        extra["void_copyright_year"] = datetime.now(tz=timezone.utc).year
        extra["void_glass"] = void["glass"]
        dot_matrix = void["dot_matrix"]
        dot_enabled = (
            dot_matrix.get("enabled", True)
            if isinstance(dot_matrix, dict)
            else dot_matrix
        )
        extra["void_dot_matrix"] = bool(dot_enabled)
        extra["void_animation"] = void["animation"]
        extra["void_border"] = void["border"]
        extra["void_highlight"] = bool(
            void["highlight"] and components["highlighting"]["show"]
        )
        extra["void_notes"] = bool(void["notes"] and components["notes"]["show"])
        extra["void_notes_ttl"] = void.get("notes_ttl")
        extra["void_components"] = components
        extra["void_keyboard"] = keyboard
        extra["void_content"] = content
        extra["void_reading_mode"] = reading_mode
        extra["void_action_cluster"] = action_cluster
        extra["void_timer"] = timer
        extra["void_config_builder"] = config_builder
        extra["void_ai_reader"] = ai_reader
        extra["void_social_cards"] = social_cards
        extra["void_meta"] = meta
        extra["void_feedback"] = feedback
        extra["void_announcement_bar"] = announcement_bar
        extra["void_cookie_consent"] = cookie_consent
        extra["void_comments"] = comments
        extra["void_consent_needed"] = consent_needed
        extra["void_i18n"] = i18n
        extra["void_breadcrumbs"] = breadcrumbs_cfg
        extra["void_pwa"] = pwa
        extra["void_assets"] = assets
        extra["void_site_url"] = raw_site_url

        # "Powered by Void" footer credit + badge — opt-in community
        # signal. `extra.void_showcase` (bool) enables it; an optional
        # `extra.void_showcase_url` overrides the default repo link.
        extra["void_showcase"] = bool(extra.get("void_showcase"))
        extra["void_showcase_url"] = str(extra.get("void_showcase_url") or "").strip()

        # Collect user-supplied design tokens. Only values the author
        # explicitly set are collected; defaults live in the compiled CSS.
        tokens = []
        for group, mapping in _VOID_TOKEN_MAP.items():
            provided = void.get(group)
            if not isinstance(provided, dict):
                continue
        ...
        # Wire theme.font so a standard MkDocs theme.font block drives
        # the body/code font stacks. Appended last so it wins over any matching
        # void.typography token the author set on the same vars.
    ...
    def _asset_fetch(self, url, site_abs_path, timeout):
        ...
        try:
            request = urllib.request.Request(
                url, headers={"User-Agent": "mkdocs-void/{}/+assets".format("1")}
            )
            ...
            _AI_LOGGER.warning(
                "void assets: could not vendor %s at build time — the "
    ...
    def _asset_rebase_urls(self, text, css_url, dest_dir, site_dir, timeout):
        ...
        for _, ref in refs:
            ...
            if not dest.startswith(site_abs + os.sep):
                _AI_LOGGER.warning(
                    "void assets: refusing to store vendored font outside the "
                    "site dir (%s).",
                    dest,
                )
                ok = False
    ...
    def on_pre_build(self, *, config):
        ...
        extra = config.get("extra") or {}
        assets = extra.get("void_assets") or {}
        mode = assets.get("mode") or _VOID_DEFAULT_ASSETS["mode"]
        if mode not in ("local", "bundle"):
            return

        components = extra.get("void_components") or {}
    ...
    def _asset_vendor(self, site_dir, extra, components, assets, originals):
        ...
        mode = assets.get("mode") or _VOID_DEFAULT_ASSETS["mode"]
        vendor = (
            str(assets.get("vendor_dir") or _VOID_DEFAULT_ASSETS["vendor_dir"])
            .strip()
            .strip("/")
        )
        if not vendor:
            vendor = str(_VOID_DEFAULT_ASSETS["vendor_dir"]).strip("/")
        timeout = int(assets.get("timeout") or _VOID_DEFAULT_ASSETS["timeout"])
        inline = bool(assets.get("inline_critical_css"))

        hljs_js = (
            originals.get(("highlighting", "cdn_url"))
            or _VOID_ASSET_CDN["highlighting_js"]
        )
        hljs_css = (
            originals.get(("highlighting", "cdn_css_url"))
            or originals.get(("highlighting", "cdn_url"))
            or _VOID_ASSET_CDN["highlighting_css"]
        )
        hljs_light = str(
            hljs.get("theme_light")
            or _VOID_DEFAULT_COMPONENTS["highlighting"]["theme_light"]
        )
        hljs_dark = str(
            hljs.get("theme_dark")
            or _VOID_DEFAULT_COMPONENTS["highlighting"]["theme_dark"]
        )
        mermaid_js = (
            originals.get(("mermaid", "cdn_url")) or _VOID_ASSET_CDN["mermaid_js"]
        )
        math_js = originals.get(("math", "cdn_url")) or _VOID_ASSET_CDN["math_js"]
        math_css = originals.get(("math", "cdn_css_url")) or _VOID_ASSET_CDN["math_css"]
        ...
        try:
            request = urllib.request.Request(
                math_css,
                headers={"User-Agent": "mkdocs-void/{}/+assets".format("1")},
            )
            with urllib.request.urlopen(request, timeout=timeout) as response:
        ...
        if changed:
            _AI_LOGGER.warning(
                "void assets: at least one vendored asset failed to download — "
        ...
        if mode == "bundle":
            ...
            if bundle_parts:
                ...
                with open(os.path.join(bundle_dir, "void-offline.js"), "wb") as handle:
                    handle.write(b"\n\n".join(bundle_parts))
        ...
        if inline:
            ...
            if os.path.isfile(light_path) and os.path.isfile(dark_path):
                for key, path in (
                    ("void_hljs_css_inline_light", light_path),
                    ("void_hljs_css_inline_dark", dark_path),
                ):
                ...
                if math_css_ok and os.path.isfile(katex_css_path):
                    with open(katex_css_path, encoding="utf-8") as handle:
                        extra["void_math_css_inline"] = handle.read()
                    extra["void_math_css_font_base"] = f"{vendor}/katex/dist/fonts/"
            else:
                _AI_LOGGER.warning(
                    "void assets: inline_critical_css skipped — vendored "
    ...
    def on_page_context(self, context, *, page, config, nav):
        ...
        extra = config.get("extra") or {}
        social_cards = extra.get("void_social_cards") or _VOID_DEFAULT_SOCIAL_CARDS
        og_image = (extra.get("void_og_image") or "").strip()
        ...
        self._social_pages.append(
            {
                "rel": rel,
                "title": (page.title or config.get("site_name") or "Void").strip(),
    ...
    def _social_logo_path(self, config):
        ...
        Priority: ``extra.void_logo_light``, then ``extra.void_logo_dark``,
        ...
        extra = config.get("extra") or {}
        candidates = [
            extra.get("void_logo_light"),
            extra.get("void_logo_dark"),
            theme.get("logo"),
        ]
    ...
    def _pwa_manifest(self, config, pwa):
        ...
        `extra.void_theme_color` / `site_url`; the icon list resolves from the
    ...
    def _pwa_icon(self, config):
        ...
        .void_logo_light`` → ``extra.void_logo_dark`` → ``theme.logo``) with
    ...
    def _pwa_fetch_icon(self, url, site_dir, index):
        ...
        try:
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "mkdocs-void/+pwa-icon"},
            )
            ...
            _AI_LOGGER.warning(
                "void pwa: could not fetch the manifest icon from %s at build "
    ...
    def on_post_page(self, output, *, page, config):
        ...
        extra = config.get("extra") or {}
        ai_reader = extra.get("void_ai_reader") or _VOID_DEFAULT_AI_READER
        ...
        if not ai_reader.get("overwrite") and rel in self._ai_claimed:
            if not self._ai_warned_overwrite:
                ...
                _AI_LOGGER.warning(
                    "void ai_reader: markdown mirror %r already has an "
                    "owner in this build; skipped. Set "
                    "theme.void.ai_reader.overwrite to true to allow "
                    "clobbering.",
                    rel,
                )
            return output
    ...
    def on_post_build(self, *, config):
        """Render the auto social-card images and emit llms.txt /
        ...
        # Auto-generate `manifest.webmanifest` (name, icons,
        ...
        pwa = extra.get("void_pwa") or _VOID_DEFAULT_PWA
        if pwa.get("manifest"):
    ...
    def _ai_watermark_lines(self, ai_reader, page, mirror_url, config):
        ...
        if not watermark.get("header"):
            return lines
        lines.append(
            f"<!-- {watermark.get('text', 'Generated by Void for AI agents.')} -->"
        )
        ...
        if watermark.get("include_version"):
            lines.append(f"<!-- Version: {_VOID_THEME_VERSION} -->")
        return lines
    ...
    def _ai_decorate_html(self, output, mirror_url):
        ...
        if body_match:
            comment = f"<!-- void-ai-readable: {mirror_url} -->\n"
            output = (
    ...
    def _ai_write_llms(self, site_dir, config, ai_reader, mirrors):
        ...
        lines = [f"# {config.get('site_name') or 'Void'}", ""]
