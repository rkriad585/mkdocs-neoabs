...
## Phase 21: Interactive Config Builder (mkdocs.yml Generator)

**Goal:** Ship an opt-in, client-side "Config Builder" — a guided wizard that
lets a developer assemble their own `mkdocs.yml` by **clicking checkboxes** for
feature toggles and **answering questions** (each option can carry a
**recommended** tag), with a live YAML preview and **Download** / **Copy**
buttons. It is **OFF by default** and intended for the **official
mkdocs-neoabs website**, but any developer can enable it on their own site from
`mkdocs.yml`. A distinctive builder icon is pinned at the **bottom of the TOC
panel** *and* mirrored into the **action cluster at all times**, so the tool is
always reachable from anywhere on the page. Presets — **Default**, **Standard**,
**Custom** — let the developer start from the theme's own defaults, a
recommended balanced setup, or a blank/current state.

> **Note on defaults:** this is the one phase where a new feature is **OFF by
> default** on purpose (the user asked for it). Every *inner* switch
> (`toc_icon`, `cluster_icon`, `download`, `copy`) still defaults to ON once the
> master switch is on.
...
theme:
  neoabs:
    config_builder:
      enabled: false                # Master switch — OFF by default (opt-in dev
                                    #   tool, intended for the official website;
                                    #   developers enable it per-site)
      toc_icon: true                # Builder icon pinned at the bottom of the TOC
                                    #   panel (inside .neoabs-toc__inner)
      cluster_icon: true            # Mirror the icon into the action cluster at
                                    #   all times (independent of TOC visibility)
      presets:
        - default                   # Preset card: the theme's own default config
        - standard                  # Preset card: recommended balanced setup
        - custom                    # Preset card: start from current selections
      download: true                # Show the "Download mkdocs.yml" button
      copy: true                    # Show the "Copy config" button
      target: "mkdocs.yml"          # Download filename
```

The builder generates real keys against the surfaces already implemented:
`site_name`/`site_description`/`repo_url`, `theme.name: neoabs`, `theme.features`,
`theme.neoabs.*` (glass, components, toc, keyboard, action_cluster, timer,
reading_mode, ai_reader, etc.), `extra.*`, `plugins` (search, neoabs), and the
checkbox set mirrors the `theme.features` + `components.*` toggles.
...
> top of the file). Violating any of these makes this phase a defect:
>
> - **Don't remove any element** — the builder adds a new dialog, a TOC trigger,
>   and a cluster action; no existing HTML, CSS rule/class, ID, or markup is
>   deleted. The TOC trigger is appended inside `.neoabs-toc__inner` (additive).
> - **Don't remove any feature** — the normal site renders exactly as before;
>   the builder is invisible (and not emitted) while `enabled: false`.
> - **Don't remove any function** — existing JS functions stay; the phase adds
>   `initConfigBuilder`, `configBuilderOpen/Close/Download/Copy` and a
>   `config_builder` entry in `ACTION_CLUSTER_ICONS` + `actionClusterDispatch`.
> - **Don't make any typo** — every class name (`.neoabs-config-builder*`,
>   `.neoabs-toc__inner`), config key (`config_builder`, `presets`, `target`),
>   and i18n alias must match the actual emitted markup and `#__config`
>   injection exactly.
> - **Don't touch any code outside this phase's topic** — only the files
>   listed in this phase may be edited; anything else is out of scope.
> - **Don't miss any feature in this phase** — implement **every** listed key
>   and behavior: checkboxes, question/option flow with recommendations, the
>   three preset cards, Download + Copy, the TOC-bottom icon, the distinct
>   dev-tool styling, and the always-on cluster mirror.
> - **Everything stays enabled/active by default** — EXCEPT this feature's
>   master `enabled`, which is **OFF by default by explicit design** (the user
>   asked for an opt-in tool). All inner switches default to ON.
> - **Verify before done** — `npm run build`, `mkdocs build --quiet`,
>   `npm test`, and the relevant harnesses must pass at the end of this
>   phase.
...
**Files to modify:**
- `neoabs/plugins/neoabs_plugin.py`:
  - Add `_NEOABS_DEFAULT_CONFIG_BUILDER` with the defaults above; deep-merge
    `theme.neoabs.config_builder` and validate with clear `ConfigurationError`
    messages (`enabled`/`toc_icon`/`cluster_icon`/`download`/`copy` booleans,
    `presets` a list of `default|standard|custom`, `target` a non-empty string).
  - Expose the merged result as `extra["neoabs_config_builder"]`.
  - Add a `config_builder` i18n group to `_NEOABS_DEFAULT_I18N` (title, open,
    close, open_label, preset_default/standard/custom, download, copy, copied,
    recommended) plus flat aliases per the Phase 7 convention.
- `neoabs/templates/base.html`:
  - Emit the config as `"config_builder": ...` inside `#__config` (alongside the
    other `neoabs_*` surfaces).
  - Include `partials/config_builder.html` before `</body>` only when
    `config.extra.neoabs_config_builder.enabled` is true.
- `neoabs/templates/partials/toc.html`:
  - When `config_builder.toc_icon` and master enabled, append a
    `<button class="neoabs-config-builder__toc-trigger" ...>` as the last child
    of `.neoabs-toc__inner` (bottom of the TOC panel). This is the "build icon".
- New `neoabs/templates/partials/config_builder.html`:
  - A dialog with a distinct developer-tool look (`neoabs-config-builder` +
    `neoabs-config-builder--dev-tool`): accent-red border, mono-font labels, a
    "DEV TOOL" badge, so it is instantly recognizable and clearly separate from
    the documentation UI.
  - Three preset cards (Default / Standard / Custom) at the top; selecting one
    resets the answers.
  - The question flow: options render as buttons/radio rows with a
    **recommended** chip (driven by schema metadata), feature toggles render as
    checkboxes; both feed the YAML model.
  - A live `<pre class="neoabs-config-builder__yaml">` preview with inline YAML
    syntax highlighting (keys / strings / booleans / numbers / comments, via a
    self-contained HTML-escaped tokenizer so it never depends on
    highlight.js).
  - Footer actions: **Download** (`a[download]` via a Blob) and **Copy**
    (`navigator.clipboard` with a `execCommand` fallback, `neoabsToast` on
    success) — each gated by its own key.
- `neoabs/templates/assets/javascripts/neoabs.js`:
  - `NEOABS_VERSION` `"22"` → `"23"`.
  - `initConfigBuilder(config)`: no-op unless `config_builder.enabled`; builds
    the question schema from `_config` + bundled schema metadata; wires the TOC
    trigger (if `toc_icon`).
  - When `cluster_icon` is on, add a `config_builder` action to the cluster menu
    (new `ACTION_CLUSTER_ICONS.builder` glyph +
    `actionClusterDispatch("config_builder")` → `configBuilderOpen`) **at all
    times** — created once and never removed, so it survives SPA nav and resize
    re-syncs. The TOC-bottom pin and the cluster icon coexist intentionally (the
    cluster mirror is not deduped by visibility).
  - `configBuilderOpen()/configBuilderClose()` with focus trap + Escape + click
    outside; `configBuilderDownload()` / `configBuilderCopy()`.
- `neoabs/templates/assets/stylesheets/components.scss`: add
  `.neoabs-config-builder*` (dialog, preset cards, recommended chip, yaml
  preview, TOC-bottom trigger, cluster mirror action) with the distinct dev-tool
  styling. **Regenerate** `neoabs/templates/assets/neoabs.css` via
  `npm run build`.
- `mkdocs.yml`: `extra.neoabs_version: 23`; add the Phase 21 reference block
  (commented) listing `config_builder` + presets. The official site enables it
  with `config_builder.enabled: true` in its own mkdocs.yml (this repo's docs
  site may enable it per the maintainers' choice — the *default* stays off).
- `PLAN.md`: this Phase 21 section + implementation-order row.
- Harness: new `phase21-harness` (temp) verifying: OFF builds emit no builder
  markup and no `config_builder` in `#__config`; ON builds emit the partial, the
  TOC trigger inside `.neoabs-toc__inner`, and `config_builder` in `#__config`;
  toggle/option schema drives the YAML model; generated YAML round-trips
  (parses under `tomllib`/PyYAML and contains real `theme.neoabs.*` keys);
  simulator coverage for `configBuilderDownload`/`configBuilderCopy`; cluster
  mirror icon always present in the cluster menu.

### Status: Implemented
Implemented per the phase specification above. Summary of the shipped state:
- **Plugin** (`neoabs/plugins/neoabs_plugin.py`): `_NEOABS_DEFAULT_CONFIG_BUILDER`
  (enabled OFF by default — the intentional exception to the default-ON rule),
  `_NEOABS_CONFIG_BUILDER_PRESETS`, `_NEOABS_CONFIG_BUILDER_BOOLS`,
  `_validate_config_builder()` (booleans, presets list, non-empty target),
  `config_scheme` entry, deep merge + `extra["neoabs_config_builder"]`,
  `config_builder` i18n group + flat aliases, and `builder`/`config_builder`
  added to the action-cluster icon/id enums.
- **base.html**: `_cb` var; `config_builder` emitted into `#__config` only when
  enabled; `partials/config_builder.html` included only when enabled;
  `data-md-neoabs-config-builder="true"` on `<html>` when enabled.
- **toc.html**: `.neoabs-config-builder__toc-trigger` appended as the last child
  of `.neoabs-toc__inner` when enabled + `toc_icon`.
- **config_builder.html** (new): dialog shell + embedded schema JSON (7 groups,
  30 questions: text / select-with-recommended / toggle / checkbox); the dialog
  is a two-pane workspace — live YAML preview column on the **left**, question
  groups on the **right**, header/presets/footer pinned.
- **neoabs.js**: version "23", `ACTION_CLUSTER_ICONS.builder`,
  `actionClusterDispatch("config_builder")`, the full Phase 21 block
  (`initConfigBuilder`, preset apply, live YAML serializer,
  `configBuilderHighlightYaml` — a self-contained HTML-escaped YAML highlighter
  with `tok-key` / `tok-str` / `tok-bool` / `tok-num` / `tok-comment` tokens so
  it never depends on highlight.js loading, open/close with focus trap + Escape
  + click-outside, download/copy, `keyboardActions.open_config_builder`). The
  cluster mirror is **always shown** when `cluster_icon` is on (independent of
  TOC visibility); it is created once and persisted across SPA nav/resize.
- **components.scss**: `.neoabs-config-builder*` dev-tool styling + regen of
  `neoabs.css`; the two-pane grid collapses below
  `$neoabs-breakpoint-toc` (columns stack, preview caps at 260px) and the panel
  goes full-bleed below `$neoabs-breakpoint-mobile`; extras/copy
  (`overscroll-behavior`, `-webkit-overflow-scrolling`) keep every pane
  independently scrollable on touch devices.
- **mkdocs.yml**: `extra.neoabs_version: 23`, Phase 21 reference block, and the
  official site enabling `config_builder.enabled: true`.
- **Harness**: `phase21-harness` checks added to `tests/neoabs.test.js` (OFF
  untouched, ON renders highlighted left-pane YAML preview via
  `dataset.yaml`/`innerHTML` tokens, open/close, clipboard copy, and the cluster
  icon present **both** when the TOC is hidden *and* visible). Verified with
  `npm run build`, `npm test` (53 checks), `mkdocs build --clean`, OFF-fixture
  build (no markup / no top-level `config_builder` key), and JSON.parse on the
  built `#__config` + schema.
...
| 21 | Low | Medium | Interactive config builder (mkdocs.yml generator; opt-in off by default, TOC-bottom + cluster icon, Default/Standard/Custom presets) |
