...
- [Added] feat: rename mkdocs-neoabs to mkdocs-void - rebrand package/theme/plugin to Void (mkdocs_neoabs->mkdocs_void, NeoAbs->Void, neoabs->void across code, templates, tests, docs, workflows, URLs, metadata, README, CONTRIBUTORS), strip Phase N comments for real function docs in plugin/JS/SCSS/templates/tests/mkdocs.yml + 14 docs refs, swap logos from void-logos/ per dark/light contract (drop old logo/, repoint raw URLs to docs/assets/images), contributor credits to rkriad585 contributors, watermark 'Generated by Void for AI agents.'; moves rename tool to tools/rename_to_void.py + plan to RENAME-PLAN.md; tests 56/56 + strict build green
...
- [Fixed] fix: correct action ref to tag 12.6.2 (no v prefix)
- [Fixed] fix: Performance workflow - pin treosh/lighthouse-ci-action@v12.6.2 (bundles @lhci/cli 0.15.1; the v12 tag bundles an older cli that cannot read category scores from Lighthouse 12 reports, failing minScore with the no-value error) and relax thresholds to measured values (mobile perf 0.69, accessibility 0.96, best-practices 1, seo 1); validated against the real report locally
- [Fixed] fix: migrate .lighthouseci.json to the v8+ ci config shape - add the top-level ci key (collect/assert/upload) required by treosh/lighthouse-ci-action v12 to replace the legacy top-level array
- [Fixed] fix: align recipe assertions/docs with re-published plugin surfaces (table-reader v3 reader functions, print-site v2.9 basename/theme-auto, section-index href from home page); validated locally, strict build green, void doctor exit 0
- [Fixed] fix: use current PyPI package names for the plugin recipe ecosystem - the mkdocs-print-site/table-reader/git-authors/awesome-pages PyPI projects 404 (external takedown); the surviving '-plugin' named packages ship the same plugin ids (print-site, table-reader, git-authors, awesome-pages) so only the pip install lines change; update integrations.md install notes + section headers, keep plugins: configs untouched; docs build strict green
- [Fixed] fix: pin recipe CI to mkdocs-git-revision-date-localized-plugin (plugin id unchanged) - the mkdocs-git-revision-date-localized PyPI project now 404s (external takedown of both package and upstream GitHub repo), so the Integrations recipe installs the surviving package name that still ships the git-revision-date-localized plugin + page.meta.git_revision_date_localized; update integrations.md/void.md/configuration.md/mkdocs.yml notes accordingly
...
- [Changed] chore: bump version to 0.2.0 for the mkdocs-void package identity
- [Changed] temporarily upload LHCI results to public storage to inspect the report
